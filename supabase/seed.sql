...
insert into public.pieces (
  ...
  sample_url, purchase_url, solo_status, solos, source, note
) values
(
...
(
  ...
  'Altsaxophon oder Trompete', 'Demodaten', null
),
(
...
on conflict (project_id, import_key) do update set
  ...
  solos = excluded.solos, source = excluded.source, note = excluded.note, updated_at = now();
