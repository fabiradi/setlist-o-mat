...
type Piece = {
  ...
  source: string; subtitle: string | null; note: string | null;
};
...
type AdminPiecePatch = Pick<Piece, "title" | "subtitle" | "composer" | "genres" | "sampleUrl" | "purchaseUrl" | "soloStatus" | "solos" | "durationSeconds" | "grade" | "priceCents" | "owned" | "source" | "note">;
...
const emptyPiece: Piece = {
  ...
  purchaseUrl: null, soloStatus: "unknown", solos: null, source: "", subtitle: null, note: null,
};
...
export default function Home() {
  ...
  }, [activeSetlistId, builderId, setlistFilter, view]);
  ...
  useEffect(() => {
    ...
    const returnHash = url.searchParams.get("returnHash") || window.localStorage.getItem(RETURN_HASH_KEY);
  ...
  useEffect(() => {
    ...
    const loadProjectData = async () => {
      ...
      if (currentDisplayName && session.user.user_metadata?.display_name !== currentDisplayName) {
        void supabase.auth.updateUser({ data: { display_name: currentDisplayName } }).then(({ error }) => {
      ...
      const mappedPieces = dbPieces.flatMap((piece) => {
        ...
        return [{
          ...
          source: piece.source, subtitle: piece.subtitle, note: piece.note,
        } as Piece];
  ...
  const queueSetlistSave = (setlist: Setlist) => {
    ...
    if (!setlist.name.trim()) { pendingSetlistSave.current = null; setSetlistSaveState("error"); return; }
  ...
  const saveProfileName = async (name: string): Promise<string | null> => {
    ...
    const { error: authMetadataError } = await supabase.auth.updateUser({ data: { display_name: normalizedName } });
    ...
    if (authMetadataError) return "Der Name ist in der App gespeichert, konnte aber nicht in die Kontometadaten übernommen werden. Bitte versuche das Speichern noch einmal.";
  ...
  const deleteSetlist = async (setlist: Setlist) => {
    ...
    const consequence = isPublished
      ...
      : "";
    if (!window.confirm(`${isPublished ? "Die veröffentlichte Setlist" : "Den Entwurf"} „${setlist.name}“ wirklich löschen?${consequence}`)) return;
    if (supabase && session && typeof setlist.id === "string") {
      const { data, error } = await supabase.from("setlists").delete().eq("id", setlist.id).select("id").maybeSingle();
      if (error || !data) { console.error("setlist delete failed", error); flash("Setlist konnte nicht gelöscht werden"); return; }
    }
    setSetlists((current) => current.filter((item) => item.id !== setlist.id));
    setSetlistRatings((current) => { const next = { ...current }; delete next[String(setlist.id)]; return next; });
    if (builderId === setlist.id || activeSetlistId === setlist.id) { setBuilderId(null); setActiveSetlistId(null); writeAppHash(setlistsHash(setlistFilter), true); }
    flash(isPublished ? "Setlist und zugehörige Rückmeldungen gelöscht" : "Entwurf gelöscht");
  };
    ...
    <section className="main-stage">
      ...
      {view === "setlists" && <div className="page setlists-page">
        ...
        <div className="setlist-tabs" role="tablist" aria-label="Setlists filtern">
          <button role="tab" aria-selected={setlistFilter === "all"} className={setlistFilter === "all" ? "active" : ""} onClick={() => { setSetlistFilter("all"); writeAppHash(setlistsHash("all"), true); }}>Alle <span>{setlists.length}</span></button>
          <button role="tab" aria-selected={setlistFilter === "finalists"} className={setlistFilter === "finalists" ? "active" : ""} onClick={() => { setSetlistFilter("finalists"); writeAppHash(setlistsHash("finalists"), true); }}>Finalrunde <span>{setlists.filter((item) => item.state === "finalist" || item.state === "final").length}</span></button>
          <button role="tab" aria-selected={setlistFilter === "mine"} className={setlistFilter === "mine" ? "active" : ""} onClick={() => { setSetlistFilter("mine"); writeAppHash(setlistsHash("mine"), true); }}>Meine Entwürfe <span>{ownDraftCount}</span></button>
        </div>
        {visibleSetlists.length ? <div className="setlist-grid">{visibleSetlists.map((setlist) => {
          ...
          const isOwnSetlist = !session || setlist.ownerId === session.user.id;
          return <article className={`setlist-card state-${setlist.state}`} key={setlist.id}>
            <div className="setlist-card-head"><div>{setlist.state === "draft" ? <Lock /> : setlist.state === "finalist" || setlist.state === "final" ? <Trophy /> : <ListMusic />}</div><span className={`status-pill ${setlist.state}`}>{setlist.state === "draft" ? isOwnSetlist ? "Mein Entwurf" : "Fremder Entwurf" : setlist.state === "finalist" ? "Finalrunde" : setlist.state === "final" ? "Finale Setlist" : "Veröffentlicht"}</span></div>
      ...
      {view === "admin" && isAdmin && <div className="page admin-page">
        <div className="page-heading"><div><span className="eyebrow"><Settings /> Adminbereich</span><h1>Alles im Takt halten.</h1><p>Metadaten vervollständigen, Teilnehmer verwalten und den Auswahlprozess steuern.</p></div></div>
...
function BuilderDialog({ catalogue, setlist, pieceRatings, groupRatings, saveState, onRetry, onClose, onDelete, onPatch, onPublish }: { catalogue: Piece[]; setlist: Setlist; pieceRatings: Record<number, Rating>; groupRatings: Record<number, GroupRating>; saveState: SetlistSaveState; onRetry: () => void; onClose: () => void; onDelete: () => void; onPatch: (patch: Partial<Setlist>) => void; onPublish: () => void }) {
  ...
  const metrics = getMetrics(setlist.pieceIds, catalogue);
  const candidates = catalogue.filter((piece) => !setlist.pieceIds.includes(piece.id) && `${piece.title} ${piece.composer}`.toLowerCase().includes(query.toLowerCase()));
...
function AdminPieceDialog({ piece, creating = false, onClose, onSave }: { piece: Piece; creating?: boolean; onClose: () => void; onSave: (patch: AdminPiecePatch) => Promise<boolean> }) {
  ...
  const [initialForm] = useState(() => ({
    ...
    solos: piece.solos ?? "",
    duration: piece.durationSeconds ? formatDuration(piece.durationSeconds) : "",
  ...
  const isDirty = title !== initialForm.title || subtitle !== initialForm.subtitle || composer !== initialForm.composer || genreText !== initialForm.genreText
    || sampleUrl !== initialForm.sampleUrl || purchaseUrl !== initialForm.purchaseUrl || soloStatus !== initialForm.soloStatus
    || solos !== initialForm.solos || duration !== initialForm.duration || grade !== initialForm.grade || price !== initialForm.price
    || owned !== initialForm.owned || source !== initialForm.source || note !== initialForm.note;
  const requestClose = () => {
    if (saving) return;
  ...
  const changeSoloStatus = (nextStatus: Piece["soloStatus"]) => {
    ...
    if (nextStatus !== "available" && solos.trim() && !window.confirm("Die eingetragenen Solo-Instrumente werden entfernt. Fortfahren?")) return;
  ...
  const save = async () => {
    ...
    const saved = await onSave({
      title: title.trim(), subtitle: subtitle.trim() || null, composer: composer.trim(),
      genres: genreText.split(",").map((item) => item.trim()).filter(Boolean),
      sampleUrl: sampleUrl.trim() || null, purchaseUrl: purchaseUrl.trim() || null,
      soloStatus, solos: soloStatus === "available" ? solos.trim() || null : null,
  ...
  return <div className="dialog-backdrop"><section className="dialog admin-dialog"><button className="dialog-close" disabled={saving} onClick={requestClose}><X /></button><span className="dialog-kicker"><Pencil /> Metadaten bearbeiten</span><h2>{piece.title}</h2><p className="dialog-subtitle">Alle Katalogdaten dieses Stücks</p><div className="form-grid"><label className="full"><span>Titel</span><input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className="full subtitle-field"><span>Untertitel</span><textarea value={subtitle} onChange={(event) => setSubtitle(event.target.value)} placeholder={"z. B. Dancing Queen\nMamma Mia\nFernando"} /></label><div className="full composer-genre-row"><label><span>Komponist / Arrangeur</span><input value={composer} onChange={(event) => setComposer(event.target.value)} /></label><label><span>Genre</span><input value={genreText} onChange={(event) => setGenreText(event.target.value)} placeholder="z. B. Film, Rock/Pop" /></label></div><fieldset className="full solo-status-field"><legend>Soli-Status</legend><div className="solo-status-picker"><button type="button" className={`solo-status-option unknown ${soloStatus === "unknown" ? "active" : ""}`} aria-pressed={soloStatus === "unknown"} onClick={() => changeSoloStatus("unknown")}><CircleHelp /><span>Noch nicht geprüft</span></button><button type="button" className={`solo-status-option none ${soloStatus === "none" ? "active" : ""}`} aria-pressed={soloStatus === "none"} onClick={() => changeSoloStatus("none")}><Check /><span>Keine Soli</span></button><button type="button" className={`solo-status-option available ${soloStatus === "available" ? "active" : ""}`} aria-pressed={soloStatus === "available"} onClick={() => changeSoloStatus("available")}><UserRound /><span>Soli vorhanden</span></button></div></fieldset>{soloStatus === "available" && <label className="full solo-instruments-field"><span>Solo-Instrumente / Hinweise <small>optional</small></span><input value={solos} onChange={(event) => setSolos(event.target.value)} placeholder="z. B. Altsaxophon, Oboe, Posaune …" /></label>}<div className="full piece-commercial-row"><label><span>Dauer (mm:ss)</span><input value={duration} onChange={(event) => setDuration(event.target.value)} /></label><label><span>Grade</span><input type="number" step="0.5" value={grade} onChange={(event) => setGrade(event.target.value)} /></label><label><span>Preis in Euro</span><input inputMode="decimal" value={price} onChange={(event) => setPrice(event.target.value)} /></label><label className="check-label"><input type="checkbox" checked={owned} onChange={(event) => setOwned(event.target.checked)} /><span>Bereits im Bestand</span></label></div><label className="full"><span>Hörprobe (YouTube-URL)</span><input type="url" value={sampleUrl} onChange={(event) => setSampleUrl(event.target.value)} placeholder="https://www.youtube.com/watch?v=…" /></label><label className="full"><span>Kauflink</span><input type="url" value={purchaseUrl} onChange={(event) => setPurchaseUrl(event.target.value)} placeholder="https://…" /></label><label className="full"><span>Quelle</span><input value={source} onChange={(event) => setSource(event.target.value)} /></label><label className="full"><span>Kommentar</span><textarea value={note} onChange={(event) => setNote(event.target.value)} /></label></div>{error && <div className="profile-error"><CircleHelp /> {error}</div>}<div className="dialog-actions"><button className="text-button" disabled={saving} onClick={requestClose}>Abbrechen</button><button className="primary-button" disabled={saving} onClick={() => void save()}><Check /> {saving ? "Wird gespeichert …" : "Speichern"}</button></div></section></div>;
}
