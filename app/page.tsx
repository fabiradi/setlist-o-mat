...
function PieceRatingBadges({ rating, groupRating }: { rating?: Rating; groupRating?: GroupRating }) {
  return <div className="piece-rating-badges">{rating ? <><span className="mine">{rating.skipped ? <><CircleHelp /> Du: nicht beurteilt</> : <><Star fill="currentColor" /> Du: {rating.stars}/5</>}</span><span className="group"><Users /> {groupRating?.count ? `Gruppe: Ø ${groupRating.average.toFixed(1).replace(".", ",")} (${groupRating.count})` : "Noch keine Gruppenwertung"}</span></> : <span className="locked"><Lock /> Gruppenwertung nach deiner Bewertung</span>}</div>;
}
