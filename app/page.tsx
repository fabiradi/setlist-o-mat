...
export default function Home() {
  ...
  return <main className="app-shell">
      ...
      {view === "setlists" && <div className="page setlists-page">
        ...
        {visibleSetlists.length ? <div className="setlist-grid">{visibleSetlists.map((setlist) => {
          ...
          return <article className={`setlist-card state-${setlist.state}`} key={setlist.id}>
            ...
            <h2>{setlist.name}</h2><p>{isOwnSetlist ? "von dir" : `von ${setlist.owner}`} · {setlist.pieceIds.length} Stücke</p><TimeSignal duration={metrics.duration} compact />
    ...
    {activeSetlist && <SetlistDialog catalogue={catalogue} setlist={activeSetlist} rating={setlistRatings[String(activeSetlist.id)]} currentUserId={session?.user.id ?? "demo-current"} canDelete={!supabase || activeSetlist.ownerId === session?.user.id || isAdmin} onCopyLink={() => void copyCurrentLink()} onClose={closeSetlist} onDelete={() => void deleteSetlist(activeSetlist)} onSave={(rating) => { void saveSetlistRating(activeSetlist, rating); closeSetlist(); }} />}
...
function SetlistDialog({ catalogue, setlist, rating, currentUserId, canDelete, onCopyLink, onClose, onDelete, onSave }: { catalogue: Piece[]; setlist: Setlist; rating?: SetlistRating; currentUserId: string; canDelete: boolean; onCopyLink: () => void; onClose: () => void; onDelete: () => void; onSave: (rating: SetlistRating) => void }) {
  const [stars, setStars] = useState<number | null>(rating?.stars ?? null);
  const [comment, setComment] = useState(rating?.comment ?? "");
  const metrics = getMetrics(setlist.pieceIds, catalogue);
