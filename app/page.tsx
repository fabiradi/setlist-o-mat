...
function PieceRatingBadges({ rating, groupRating }: { rating?: Rating; groupRating?: GroupRating }) {
  return <div className="piece-rating-badges">{rating ? <><span className="mine"><span className="rating-owner-icon" title="Du" aria-label="Du"><UserRound /></span>{rating.skipped ? <small>Nicht beurteilt</small> : <DisplayStars value={rating.stars ?? 0} />}</span><span className="group"><span className="rating-owner-icon" title="Gruppe" aria-label="Gruppe"><Users /></span>{groupRating?.count ? <><DisplayStars value={groupRating.average} /><small>Ø {groupRating.average.toFixed(1).replace(".", ",")} · {groupRating.count}</small></> : <small>Noch keine Gruppenwertung</small>}</span></> : <span className="locked"><Lock /> Gruppenwertung nach deiner Bewertung</span>}</div>;
}
