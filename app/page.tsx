...
function SetlistPlayer({ pieces, compact = false, activePieceId, onActivePieceChange }: { pieces: Piece[]; compact?: boolean; activePieceId: number | null; onActivePieceChange: (pieceId: number) => void }) {
  ...
  useEffect(() => {
    ...
    if (index >= 0) playerRef.current?.cuePlaylist(playable.map((piece) => piece.youtubeId), index, 0);
  ...
  useEffect(() => {
    activePieceIdRef.current = activePieceId;
    if (activePieceId === null || !playable.length || !playerHostRef.current) return;
    let cancelled = false;
    const videoIds = playable.map((piece) => piece.youtubeId);
    const initialIndex = playable.findIndex((piece) => piece.id === activePieceId);
      ...
      playerRef.current = new YT.Player(playerHostRef.current, {
        ...
        events: {
          ...
          onStateChange: (event) => {
            ...
            if (index >= 0 && playable[index]) reportActivePiece(playable[index].id, true);
          },
        },
      });
  ...
  const activePiece = playable.find((piece) => piece.id === activePieceId) ?? null;
